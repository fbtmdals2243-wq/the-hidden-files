...
function runSearch(){
 ...
 if(!q)return;
 const results=all().filter(x=>JSON.stringify(x).toLowerCase().includes(q));
 showPage("viewer");
 detail.innerHTML=`<div class="card"><h1>Search Results</h1><div class="list">${
  results.length?results.map(x=>`<div class="item" onclick="playSound('click');openRecord('${x.id}')"><b>${x.id} · ${x.name||x.title}</b><br><span class="muted">${x.clearance||x.classification||x.status}</span></div>`).join(""):"<p class='muted'>No recovered records found.</p>"
 }</div></div>`;
}
